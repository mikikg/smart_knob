...
void USART1_IRQHandler(void)
{
  ...
  /* USER CODE BEGIN USART1_IRQn 1 */
  	...
  	uint16_t IIR = USART1->SR; //remember state
    //------------- IDLE interupt ----------
    if (IIR & USART_SR_IDLE) {
		...
		//GLOABAL_DIAG[RECEIVED_PACKET] ++; //za diagnostiku
        UART_KBD_rx_ready=1; //kraj prenosa sa KBD UART_KBD_RX_packet_len UART_KBD_RX_buff
        HAL_GPIO_TogglePin(LED_GPIO_Port, LED_Pin);
    }

    //------------- Transfer Complete interupt ----------
    if (IIR & USART_SR_TC) {  //tc interupt
        USART1->SR &= ~USART_SR_TC;	//ocisti TC flag
		//GLOABAL_DIAG[TRANSMITTED_PACKET] ++; //za diagnostiku
    }
    ...
    if (IIR & USART_SR_RXNE) {          // read interrupt
        ...
        if ((IIR & USART_SR_FE) == 2)  { 	//is framing error
            //GLOABAL_DIAG[FRAMING_ERROR] ++; //za diagnostiku
        } else {
            if (IIR & USART_SR_NE) { //noise flag
                //GLOABAL_DIAG[NOISE_ERROR] ++; //za diagnostiku
            }
            //ok, smesti karakter u prijemni buffer
            UART_KBD_RX_buff[UART_KBD_cntx++] = USART1->DR;
